...
export default function AnimationCanvas({
  ...
  useEffect(() => {
    ...
    animationData.timeline.forEach((anim) => {
      const targetEl = svgRef.current.querySelector(
        "#" + CSS.escape(anim.target),
      );
      if (!targetEl) return;
    ...
    <div className="flex items-center justify-between p-4 border-b border-white/5 bg-zinc-900/40 shrink-0">
      ...
      <div className="flex items-center gap-2">
        ...
        >
          ...
          <option value="20" className="bg-zinc-900">
            20s
          </option>
          <option value="25" className="bg-zinc-900">
            25s
          </option>
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-4 p-8 text-center animate-fade-in opacity-60 hover:opacity-100 transition-opacity">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-500 max-w-xs">
              Describe an animation on the left and hit Generate to see the
              magic happen.
            </p>
          </div>
        </div>
  ...
  return (
    <div className="flex flex-col h-full bg-zinc-900/20">
      ...
      <div className="flex-1 flex items-center justify-center p-8 overflow-hidden bg-[url('data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAoAAAAKCAYAAACNMs+9AAAAAXNSR0IArs4c6QAAACVJREFUKFNjZCASMDKgAnv37v3/n0QwDMQpoBiNR+EokOQ2QAEAhT4Kj3xT0bYAAAAASUVORK5CYII=')]">
        <div
          className="relative group shadow-2xl shadow-purple-900/20 rounded-xl overflow-hidden border border-white/10 transition-transform duration-500 hover:scale-[1.01]"
          style={{
            width: "100%",
...
function buildGsapVars(anim) {
  ...
  if (anim.type === "move") {
    const fromVars = {},
      toVars = { ...toBase };
    if (anim.from) {
      if (anim.from.x !== undefined)
        fromVars.attr = { ...fromVars.attr, cx: anim.from.x, x: anim.from.x };
      if (anim.from.y !== undefined)
        fromVars.attr = { ...fromVars.attr, cy: anim.from.y, y: anim.from.y };
    }
    if (anim.to) {
      if (anim.to.x !== undefined)
        toVars.attr = { ...toVars.attr, cx: anim.to.x, x: anim.to.x };
      if (anim.to.y !== undefined)
        toVars.attr = { ...toVars.attr, cy: anim.to.y, y: anim.to.y };
    }
    return { from: fromVars, to: toVars };
  }
  if (anim.type === "fade")
    return {
      from: { opacity: anim.from?.opacity ?? 0 },
      to: { ...toBase, opacity: anim.to?.opacity ?? 1 },
    };
  if (anim.type === "scale")
    return {
      from: { scaleX: anim.from?.scaleX ?? 1, scaleY: anim.from?.scaleY ?? 1 },
      to: {
        ...toBase,
        scaleX: anim.to?.scaleX ?? 1,
        scaleY: anim.to?.scaleY ?? 1,
        transformOrigin: "50% 50%",
      },
    };
  if (anim.type === "rotate")
    return {
      from: { rotation: anim.from?.rotation ?? 0 },
      to: {
        ...toBase,
        rotation: anim.to?.rotation ?? 360,
        transformOrigin: "50% 50%",
      },
    };
  if (anim.type === "color")
    return {
      from: { attr: { fill: anim.from?.fill || "#ffffff" } },
      to: { ...toBase, attr: { fill: anim.to?.fill || "#000000" } },
    };
